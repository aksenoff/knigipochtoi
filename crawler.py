...
for href, category in categories.items():
    ...
    for href in hrefs:
        ...
        for href in hrefs:
            ...
            con.execute(u'insert into Книга(Категория, Название, Автор, ISBN, Переплет, Год_издания, Количество_страниц, Цена, Аннотация, Обложка) '
                        'values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                        [ category, title, authors, ISBN, cover, year, pages, price, description, image])
    con.commit()